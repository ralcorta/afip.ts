...
Este servicio de autenticación solo puede ser llamado cada 2 minutos en los servidores de producción y hasta 10 minutos en los servidores de homologiación.

Para esto otros paquetes internamente generan archivos donde guardan estos tokens de manera local en el servidor usando el file system de Node (require('fs')). Esto es un problema cuando se ejecuta sobre funciones serverless como las Lambdas de AWS.
